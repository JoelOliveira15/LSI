...
//TABELAS: bdusuario,
   ...
   @mysql_select_db($db);
	...
	//cria tabela sintomas																	nome,mensagem
	$sintomas="CREATE TABLE $sintomas(id INT NOT NULL AUTO_INCREMENT,PRIMARY KEY(id), nome varchar(50), mensagem varchar(254))";     
	#Executa query tabela
	if (mysqli_query($con,$$sintomas))  {
		
	}
	else{
	  echo "<center>Error ao criar tabela sintomas: " . mysqli_error($con);
	}

		//cria tabela especialistas																	nome,especialidade, crm
	$especialista="CREATE TABLE $especialista(id INT NOT NULL AUTO_INCREMENT,PRIMARY KEY(id), nome varchar(50), especialidade varchar(100), crm int(11))";     
	#Executa query tabela
	if (mysqli_query($con,$$especialista))  {
		
	}
	else{
	  echo "<center>Error ao criar tabela especialidade: " . mysqli_error($con);
	}

	//cria tabela transtorno																	nome,especialidade, crm
	$transtorno="CREATE TABLE $transtorno(id INT NOT NULL AUTO_INCREMENT,PRIMARY KEY(id), nome varchar(254), especialidade varchar(254))";     
	#Executa query tabela
	if (mysqli_query($con,$$transtorno))  {
		
	}
	else{
	  echo "<center>Error ao criar tabela transtorno: " . mysqli_error($con);
	}
